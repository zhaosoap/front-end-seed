...
def make_dataset_roc(df, enc, locList, idList, gongcan):
    ...
    print columns    
    return t, label, np.hstack((df.values, idAndLoc)), columns, df[['Longitude', 'Latitude']]
...
def make_dataset(df, gongcan):
    ...
    return t, label, newdf.values, newdf.columns, newdf.loc[:, ['Longitude', 'Latitude']].values
...
def run(trainPath, testPath, config):
    gcpath = "data/2g_gongcan.csv"
    gongcan = pd.read_csv(gcpath)
    #gongcan = prepare_gongcan(gongcanDF)
    trainPath = "data/clean/forward.csv"
    trainDF = pd.read_csv(trainPath)
    #trainDF = pd.read_csv('xietu_train.csv')

    testPath = "data/clean/backward.csv"
    ...
    outDf = pd.DataFrame()
    outDF['time'] = te_time
    outDF['predict'] = te_pred
    outDF['real'] = te_label
    outDF['Serving Base'] = te_base 
