...
from ...utils.grid import RoadGrid
...
ex_alg_RF_classify = Experiment('ALG_RF_classify_expt')
...
def run(trainPath, testPath, config,outPath):
    ...
    rg = RoadGrid(tr_label, config['grid_size'])
    tr_label_ = rg.transform(tr_label, False)

    est_1 = RandomForestClassifier(
        n_jobs=config['n_jobs'],
        #max_depth=5,
        n_estimators = config['n_estimators'],
        max_features=config['max_features'],
        bootstrap=config['bootstrap'],
        criterion=config['criterion']).fit(tr_feature, tr_label_)
    ...
    importance = est_1.feature_importances_
    # print importance
    ...
    f_imp.write('\n')
    f_imp.close()
    te_pred = est_1.predict_proba(te_feature)
    top_n = config['top_n']
    ...
    fl = Filter()
    te_pred = fl.mean_filter(te_pred_, config['filter_win_size'], config['filter_iters'])
