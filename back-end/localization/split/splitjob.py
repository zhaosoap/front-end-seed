...
def get_split_file_name(criteria, cleanPath):
	...
	trainFileName = 'train_' + str(num) + '_' + c1 + '_' + str(ratio) + '.csv'
	testFileName = 'test_' + str(num) + '_' + c1 + '_' + str(ratio) + '.csv'
...
def run_split(cleanFilePath, trainFileName, testFileName, criteria):
	...
	if not isRandom:
		...
		rs = cross_validation.ShuffleSplit(cleanFileRows, n_iter = 1, train_size = ratio,test_size = 1-ratio)
...
def do_split(criteria):
	...
	cleanFilePath = 'data/clean/' + cleanFile
